...
它把本地高强度推理和 Pro 审查视为两个不同层级。本地 `high`、`xhigh`、`超高` 或类似推理模式可以帮助准备和审查 gate，但绝不能替代真实 Pro 模型回复。
...
Codex 不能因为当前本地运行使用了高强度推理就跳过 gate。

- 本地 `high`、`xhigh`、`超高`、“deep reasoning” 或类似 effort 设置仍然只是本地 Codex 推理。
- Pro 指通过 Chrome 咨询外部 Pro 模型会话，或用户明确粘贴回来供审查的 Pro 回复。
- 本地高强度答案不能算作 Pro 回复，也不能让 gate 通过。
- 本地高强度推理只能用于准备 Pro brief、核查来源和审查 Pro 答案。
- 只有在明确规则下才能跳过或阻塞 gate：用户禁用、任务简单且本地、隐私原因无法分享且用户未批准脱敏 prompt、Chrome/工具发现失败，或用户明确批准不经 Pro 继续。
...
- 已经通过 Chrome 获得真实 Pro 回复，或用户明确粘贴了 Pro 回复；仅有本地 `high`、`xhigh` 或 `超高` 推理不够。
- Pro 回复直接回答了用户的真实目标。
- 关键约束、文件、数据和上下文都被考虑到。
- 关键结论没有仅依赖无来源断言。
